...
def write_config_header(size, reshape, pes):
    """Overwrite dse_config.h with the requested architecture parameters."""
    content = (
        "#ifndef DSE_CONFIG_H\n"
        "#define DSE_CONFIG_H\n"
        f"#define NUM_PES {pes}\n"
        f"#define MAX_ROWS {size}\n"
        f"#define MAX_COLS {size}\n"
        f"#define RESHAPE_FACTOR {reshape}\n"
        "#endif\n"
    )
