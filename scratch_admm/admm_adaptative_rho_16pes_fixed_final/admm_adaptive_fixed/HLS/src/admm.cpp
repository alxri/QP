...
#pragma HLS INLINE
    ...
    float prim_res_scaled = r_prim / (prim_res_norm + 1e-15f);

    float dual_res_norm = hls::fmax(norm_q, hls::fmax(ATy_norm, Px_norm));
    float dual_res_scaled = r_dual / (dual_res_norm + 1e-15f);

    // 2. Compute the scaling factor (S) with square root
    float S = hls::sqrt(prim_res_scaled / hls::fmax(dual_res_scaled, 1e-15f)); 
