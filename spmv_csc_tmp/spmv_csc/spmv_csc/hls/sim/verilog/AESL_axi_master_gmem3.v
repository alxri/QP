...
//------------------------Parameter----------------------
`define TV_IN_gmem3 "../tv/cdatafile/c.spmv_csc.autotvin_gmem3.dat"
`define TV_OUT_gmem3 "../tv/rtldatafile/rtl.spmv_csc.autotvout_gmem3.dat"
...
parameter gmem3_transaction_depth = 32'd 1024;
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > gmem3_mem_depth) begin
                $display ("/home/romoirib/QP/spmv_csc_tmp/spmv_csc/spmv_csc/hls/sim/verilog/AESL_axi_master_gmem3.v: Write request address %d exceed AXI master gmem3 array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, gmem3_mem_depth); 
                $finish;
            end
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > gmem3_mem_depth) begin
                $display ("/home/romoirib/QP/spmv_csc_tmp/spmv_csc/spmv_csc/hls/sim/verilog/AESL_axi_master_gmem3.v: Read request address %d exceed AXI master gmem3 array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, gmem3_mem_depth); 
                $finish;
            end
...
integer gmem3_port0_fresh_depth[0:1];
integer gmem3_port0_blank_depth[0:1];

initial begin : runtime_depth_init_process
    gmem3_port0_fresh_depth[0] = 1024;
    gmem3_port0_blank_depth[0] = 0;
end
//------------------------Read file------------------------ 
...
initial begin : read_file_process 
  ...
  while (trans_depth != 64'h5a5aa5a50f0ff0f0)  begin
      ...
      for(i = 0; i < gmem3_transaction_depth; i = i + 1) begin 
              if (port_idx == 0) begin
                  if (port_read_done[port_idx] == 1'b0 && read_idx == gmem3_port0_fresh_depth[transaction_num]) begin
          ...
          ret = $fread(token_tmp,fp);
                  ...
                  if (port_fill_done[port_idx] == 1'b0 && port_read_done[port_idx] == 1'b1 && read_idx == gmem3_port0_blank_depth[transaction_num]) begin
...
integer gmem3_cur_trans_depth[0:1];

initial begin : out_depth_init_process
    gmem3_cur_trans_depth[0] = 1024;
end
endmodule
