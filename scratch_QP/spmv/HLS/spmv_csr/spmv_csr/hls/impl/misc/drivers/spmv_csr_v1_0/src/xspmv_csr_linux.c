...
typedef struct {
    ...
    XSpmv_csr_uio_map maps[ MAX_UIO_MAPS ];
} XSpmv_csr_uio_info;

/***************** Variable Definitions **************************************/
static XSpmv_csr_uio_info uio_info;
...
static int uio_info_read_name(XSpmv_csr_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XSpmv_csr_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XSpmv_csr_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XSpmv_csr_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XSpmv_csr_Initialize(XSpmv_csr *InstancePtr, const char* InstanceName) {
	XSpmv_csr_uio_info *InfoPtr = &uio_info;
