...
{
	// Match the HW accumulation order used in dot_prod.cpp (interleaved partial sums)
	// so C/RTL co-sim compares like-for-like.
				...
				bool verbose = false)
{
	...
	float dummy_x = 0.0f;
	float dummy_y = 0.0f;
	float *x_ptr = (size == 0 || x.data() == nullptr) ? &dummy_x : const_cast<float *>(x.data());
	float *y_ptr = (size == 0 || y.data() == nullptr) ? &dummy_y : const_cast<float *>(y.data());

	float result_mem[1];
	result_mem[0] = std::numeric_limits<float>::quiet_NaN();

	dot_prod(x_ptr, y_ptr, size, result_mem);
	const float result = result_mem[0];

	const float golden = dot_prod_ref(x_ptr, y_ptr, size);
	const bool pass = nearly_equal(result, golden);
	if (verbose || !pass) {
		std::cout << std::setprecision(10);
		std::cout << (pass ? "CASE" : "FAIL") << ": " << (name ? name : "(unnamed)")
				  << " size=" << size << " HW=" << result << " SW=" << golden
