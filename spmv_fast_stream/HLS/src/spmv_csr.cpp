...
#pragma HLS DATAFLOW
    ...
    // Set stream depths to prevent stalls
#pragma HLS STREAM variable=values_fifo depth=1024
#pragma HLS STREAM variable=cols_fifo depth=1024
#pragma HLS STREAM variable=results_fifo depth=1024
...
#pragma HLS PIPELINE
        ...
        for (int j = 0; j < II; j++) {
            ...
            if (row_counter > row_length) {
                ...
                col   = cols_fifo.read();
                term[j] = value * x[col]; // Random access read!
            }
        }
...
{
    // AXI Master Interfaces (Data channels to DDR)
    ...
    // AXI-Lite Slave Interfaces (Control channels for CPU/PYNQ)
    ...
    // Completely partition the BRAM so the unrolled j-loop can do 9 parallel reads
    #pragma HLS ARRAY_PARTITION variable=x_local complete 
    
    for (int i = 0; i < SIZE; i++) {
        #pragma HLS PIPELINE II=1
        x_local[i] = x[i]; // Sequential burst read from AXI (Very fast)
    }
...
#pragma HLS PIPELINE
    ...
    spmv_kernel(rows_length, rows_length_pad, cols, values, y, x_local, new_nnz);
}